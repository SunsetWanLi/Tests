	...
	namespace oned {
		...
		int* Code128Reader::findStartPattern(Ref<BitArray> row){
            ...
            // Look for whitespace before start pattern, >= 50% of width of start pattern
						if (bestMatch >= 0 &&
                row->isRange(zxing::common::detector::math_utils::max(0, patternStart - (i - patternStart) / 2), patternStart,
							    false)) {
...
//				memcpy(pattern, CODE_PATTERNS[d], countersLength);
        ...
        if (!row->isRange(nextStart,
                          zxing::common::detector::math_utils::min(width, nextStart + (nextStart - lastStart) / 2),
                          false)) {
          throw ReaderException("");
        }
