...
//	{
		...
		void Geo_Scale::normalize(Geo_Scale & scale)const
		{
			...
			switch(scale.direction)
			{
				...
				case 'O':
					...
					break;
				case 'A':
					if(scale.degree > 90 || scale.degree < -90 
						|| ( (abs(scale.degree - 90) < MIN_DEGREE_DELTA || abs(scale.degree + 90) < MIN_DEGREE_DELTA)
								&& ( abs(scale.minute - 0) > MIN_MINUTE_DELTA || abs(scale.second - 0) > MIN_SECOND_DELTA) )
		...
		bool Geo_Scale::operator == (const Geo_Scale & cmp)const
		{
			if( (direction == 'O') && (cmp.direction == 'O')
				&& ( abs(minute - 0) < MIN_MINUTE_DELTA)  && ( abs(cmp.minute - 0) < MIN_MINUTE_DELTA)
				&& ( abs(second - 0) < MIN_SECOND_DELTA) && ( abs(cmp.second - 0) < MIN_SECOND_DELTA)
				)
				return abs( (degree - floor((degree + 180) / 360) * 360)
				- (cmp.degree - floor((cmp.degree + 180) / 360) *360) )
				< MIN_DEGREE_DELTA;
			else
				return  (direction == cmp.direction)
				&&	( abs(degree - cmp.degree) < MIN_DEGREE_DELTA)
				&&  ( abs(minute - cmp.minute) < MIN_MINUTE_DELTA)
				&&  ( abs(second - cmp.second) < MIN_SECOND_DELTA)
				;
		}
		...
		bool Geo_Scale::operator > (const Geo_Scale & cmp)const
		{
			...
			if( abs(degree - cmp.degree) > MIN_DEGREE_DELTA)
				return degree > cmp.degree;
			if( abs(minute - cmp.minute) > MIN_MINUTE_DELTA)
...
//	{
		...
		{
			unsigned long x_delta = abs(  double( (x_end > region.x_end) ? x_end : region.x_end) 
										- double( (x_start < region.x_start) ? x_start : region.x_start) );
			unsigned long y_delta = abs(  double( (y_end > region.y_end) ? y_end : region.y_end)
										- double( (y_start < region.y_start) ? y_start : region.y_start) );
			unsigned long x_length = abs(double(x_end - x_start)) + abs(double(region.x_end - region.x_start));
			unsigned long y_length = abs(double(y_end - y_start)) + abs(double(region.y_end - region.y_start));
