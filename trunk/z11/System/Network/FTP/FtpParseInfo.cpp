...
#include "FtpParseInfo.h"
    ...
    int FtpParseInfo::ftpparse(FtpParseInfo &fp, char *buf, int len)
    {
        int i;
        int j;
        int state;
        ...
        switch(*buf) {
            ...
            case '+':
                ...
                for (j = 1;j < len;++j) {
                    if (buf[j] == 9) {
                        fp.name = std::string(buf + j + 1, len - j - 1);
                        return 1;
                    }
                    if (buf[j] == ',') {
                        switch(buf[i]) {
                            ...
                            case 'i':
                                ...
                                fp.id = std::string(buf + i + 1, j - i - 1);
                    ...
                    if (buf[i - 3] == 'D')
                        if (buf[i - 2] == 'I')
                            if (buf[i - 1] == 'R') {
                                // to be continued...
                                // fp.namelen -= 4;
                                fp.name = fp.name.substr(0, fp.name.size()-4);
                                fp.flagtrycwd = 1;
                            }
            if (!fp.flagtrycwd)
                fp.flagtryretr = 1;
        ...
        if ((*buf >= '0') && (*buf <= '9')) {
            ...
            fp.name = std::string(buf + j, len - j);
