...
/* Note: this does not properly handle chunks that are > 64K under DOS */
{
   ...
   /* There should be at least one zero (the compression type uint_8)
...
png_handle_sCAL(png_structp png_ptr, png_infop info_ptr, png_uint_32 length)
{
   ...
   ep = png_ptr->chunkdata + 1;        /* Skip unit uint_8 */
...
png_handle_zTXt(png_structp png_ptr, png_infop info_ptr, png_uint_32 length)
{
   ...
   else
   {
       ...
       text++;        /* Skip the compression_method uint_8 */
   }
...
defined(PNG_USER_TRANSFORM_PTR_SUPPORTED)
   ...
   /* Calculate the maximum bytes needed, adding a uint_8 and a pixel
    * for safety's sake
    */
