...
#include "MILayoutManager.h"
#include "../../CommonBase/NonCopyable.h"
...
// when adding them. For example, the following forces a MIView to have
// a horizontal and vertical alignment of leading regardless of that defined
// for the column:
// layout->AddView(v1, 1, 1, LEADING, LEADING);
//
// If the MIView using GridLayout is given a size bigger than the preferred,
...
//
// AddView takes care of adding the MIView to the MIView the GridLayout was
// created with.
namespace z11
{
	...
	class MIGridLayout : public MILayoutManager,virtual private NonCopyable
	{
	public:
		...
		explicit MIGridLayout(MIView* host);
		virtual ~MIGridLayout();

		// Creates a GridLayout with kPanel*Margin insets.
		static MIGridLayout* CreatePanel(MIView* host);
		...
		MIColumnSet* AddColumnSet(int id);
		...
		void AddView(MIView* view);

		// Adds a view using the default alignment from the column.
		// As a convenience this adds the view to the host. The view becomes owned
		// by the host, and NOT this GridLayout.
		void AddView(MIView* view, int col_span, int row_span);
		...
		// is the same as the MIView supplied in the constructor.
		...
		virtual void ViewAdded(MIView* host, MIView* view);

		// Notification that a view has been removed.
		virtual void ViewRemoved(MIView* host, MIView* view);

		// Layouts out the components.
		virtual void Layout(MIView* host);

		// Returns the preferred size for the GridLayout.
		virtual MGSize GetPreferredSize(MIView* host);

		virtual int GetPreferredHeightForWidth(MIView* host, int width);
	...
	private:
		...
		void SizeRowsAndColumns(bool layout, int width, int height, MGSize* pref);
		...
		MIColumnSet* GetColumnSet(int id);
		...
		MIColumnSet* GetLastValidColumnSet();
		...
		MIColumnSet* current_row_col_set_;
		...
		// Set to true when adding a MIView.
		bool adding_view_;
		...
		// ColumnSets.
		std::vector<MIColumnSet*> column_sets_;
	...
	class MIColumnSet:virtual private NonCopyable
	{
	public:
		~MIColumnSet();
		...
		void AddColumn(MIGridLayout::Alignment h_align,
