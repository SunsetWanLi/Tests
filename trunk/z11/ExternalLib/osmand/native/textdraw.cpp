...
bool calculatePathToRotate(RenderingContext* rc, TextDrawInfo* p) {
	...
	std::vector<SkPoint> points(len);//SkPoint points[len];
	p->path->getPoints(&points[0], len);//p->path->getPoints(points, len);
...
{
	static SkTypeface* serif = SkTypeface::CreateFromName("Droid Serif", SkTypeface::kNormal);
	return serif;
}
