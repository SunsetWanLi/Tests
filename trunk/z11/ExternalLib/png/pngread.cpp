...
png_build_index(png_structp png_ptr)
{
   ...
   rp = (png_bytep)png_malloc(png_ptr, png_ptr->rowbytes);

   png_indexp index = (png_indexp)png_malloc(png_ptr, sizeof(png_index));
   png_ptr->index = index;
   ...
   for (p = 0; p < pass_number; p++)
   {
      ...
      index->pass_line_index[p] =(png_line_indexp*)
      ...
      {
         png_line_indexp line_index = (png_line_indexp)png_malloc(png_ptr, sizeof(png_line_index));
         index->pass_line_index[p][i] = line_index;

         line_index->z_state = (z_streamp)png_malloc(png_ptr, sizeof(z_stream));
         inflateCopy(line_index->z_state, &png_ptr->zstream);
         line_index->prev_row = (png_bytep)png_malloc(png_ptr, row_byte_length);
         png_memcpy_check(png_ptr,
                           ...
                           voidp params)
{
    ...
    * uint_8 into separate bytes (useful for paletted and grayscale images).
    */
   ...
   /* Swap bytes of 16 bit files to least significant uint_8 first
    */
