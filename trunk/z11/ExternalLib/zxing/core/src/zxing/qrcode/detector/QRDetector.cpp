...
Ref<AlignmentPattern> Detector::findAlignmentInRegion(float overallEstModuleSize, int estAlignmentX, int estAlignmentY,
  ...
  int alignmentAreaLeftX = zxing::common::detector::math_utils::max(0, estAlignmentX - allowance);
  int alignmentAreaRightX = zxing::common::detector::math_utils::min((int)(image_->getWidth() - 1), estAlignmentX + allowance);
  if (alignmentAreaRightX - alignmentAreaLeftX < overallEstModuleSize * 3) {
      throw zxing::ReaderException("region too small to hold alignment pattern");
  }
  int alignmentAreaTopY = zxing::common::detector::math_utils::max(0, estAlignmentY - allowance);
  int alignmentAreaBottomY = zxing::common::detector::math_utils::min((int)(image_->getHeight() - 1), estAlignmentY + allowance);
