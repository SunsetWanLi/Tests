...
namespace zxing {
  namespace oned {
    ...
    bool UPCEANReader::findGuardPattern(Ref<BitArray> row, int rowOffset, bool whiteFirst,
      ...
      std::vector<int> counters(patternLength,0);//int counters[patternLength];
    ...
    int UPCEANReader::decodeDigit(Ref<BitArray> row, int counters[], int countersLen, int rowOffset,
      ...
      switch (patternType) {
        case UPC_EAN_PATTERNS_L_PATTERNS:
          max = L_PATTERNS_LEN;
          for (int i = 0; i < max; i++) {
            std::vector<int> pattern(countersLen,0);//int pattern[countersLen];
        ...
        case UPC_EAN_PATTERNS_L_AND_G_PATTERNS:
          ...
          for (int i = 0; i < max; i++) {
            std::vector<int> pattern(countersLen,0);//pattern[countersLen];
            for(int j = 0; j< countersLen; j++){
