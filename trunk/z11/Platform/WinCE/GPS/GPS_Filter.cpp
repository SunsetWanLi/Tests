...
#include "GPS_Info.h"
#include <math.h>
//#include "../Common1/CRS/GlobalCrt.h"
//#include "GlobalCrt.h"
//#include "GlobalCrt.h"
...
double GPS_Filter::getDistance(const GPS_Info &info)
{
	...
	double distance = sqrt(double((prj_x - last_x) * (prj_x - last_x) + (prj_y - last_y) * (prj_y - last_y)));
	return distance;
}
...
float GPS_Filter::getAcceleration(const GPS_Info &info)
{
	...
	// zgr, 2008-01-31
	float new_north = info.speed * cos(info.heading / /*360.0*/180.0 * PI);
	float new_east = info.speed * sin(info.heading / /*360.0*/180.0 * PI);
	
	float old_north = m_last_used.speed * cos(m_last_used.heading / /*360.0*/180.0 * PI);
	float old_east = m_last_used.speed * sin(m_last_used.heading / /*360.0*/180.0 * PI);
...
bool GPS_Filter::isTooFar(const GPS_Info &info)
{
	...
	double distance = getDistance(info);
	double avg_speed = (info.speed + m_last_used.speed) * 1.85 / 3.6 / 2;
	int time = max((info.utc_time.mm * 60 + info.utc_time.ss - m_last_used.utc_time.mm * 60 - m_last_used.utc_time.ss), 1);
	//return distance > /*m_para.m_distance_impossible*/ avg_speed * time * 5; // zgr, 如果两点距离超过两点可能位移的5倍，则太远，5倍是假设值，待实践

	if(distance <= /*m_para.m_distance_impossible*/ avg_speed * time * 5)
		return false;
	...
	distance = sqrt(double((prj_x - last_x) * (prj_x - last_x) + (prj_y - last_y) * (prj_y - last_y)));
	avg_speed = (info.speed + m_last_info.speed) * 1.85 / 3.6 / 2;
	time = max((info.utc_time.mm * 60 + info.utc_time.ss - m_last_info.utc_time.mm * 60 - m_last_info.utc_time.ss), 1);
...
float GPS_Filter::getHistoryAvgHeading()
{
	...
	size_t i = 0, size = m_history_gpsinfo.size();
