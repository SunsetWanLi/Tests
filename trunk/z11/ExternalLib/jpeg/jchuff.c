...
typedef struct {
   ...
   */
  JOCTET * next_output_byte;	/* => next uint_8 to write in buffer */
  size_t free_in_buffer;	/* # of uint_8 spaces remaining in buffer */
...
/* Emit a uint_8, taking 'action' if must suspend. */
...
/* Emit some bits; return TRUE if successful, FALSE if must suspend */
{
  ...
  while (put_bits >= 8) {
    ...
    if (c == 0xFF) {		/* need to stuff a zero uint_8? */
...
{
  if (! emit_bits_s(state, 0x7F, 7)) /* fill any partial uint_8 with ones */
    return FALSE;
...
{
  emit_bits_e(entropy, 0x7F, 7); /* fill any partial uint_8 with ones */
