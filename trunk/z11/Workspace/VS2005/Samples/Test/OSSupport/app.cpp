...
INT WINAPI WinMain( HINSTANCE hInst, HINSTANCE, LPTSTR szCmd, INT )
{
		...
		while(true)
		{
			...
			file->open("/Storage Card/MobileApp/GIS_Embedded/data/7_HZ_ciyaojiedao_STATIC.dat", std::ios::in | std::ios::binary);
			file->read(reinterpret_cast<char*>(&m_pageSize), sizeof(size_t));
			m_pageSize = 0;
			++num;
		}*/
