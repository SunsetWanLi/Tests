...
SHARED_PTR<RouteSegment> findRouteSegment(int px, int py, RoutingContext* ctx) {
	...
	ElapsedTimer loadTileDataTimer;
	loadTileDataTimer.start();
	vector<SHARED_PTR<RouteDataObject> > dataObjects;
	ctx->loadTileData(px, py, 17, dataObjects);
	if (dataObjects.size() == 0) {
		ctx->loadTileData(px, py, 15, dataObjects);
	}
	loadTileDataTimer.pause();
	osmand_log_print(LOG_WARN, "[Native] loadTileData %d",loadTileDataTimer.getElapsedTime());
	osmand_log_print(LOG_WARN, "[Native] loadHeads %d",ctx->timeToLoad.getElapsedTime());
	osmand_log_print(LOG_WARN, "[Native] accept line %d",ctx->userTimer.getElapsedTime());
...
vector<RouteSegmentResult> searchRouteInternal(RoutingContext* ctx, bool leftSideNavigation) {
	...
	t3.pause();
	osmand_log_print(LOG_WARN, "[Native] searchRouteInternal r3 %d",t3.getElapsedTime());
...
int RouteSegmentResult::getStartPointIndex()
{
	return startPointIndex;
}

int RouteSegmentResult::getEndPointIndex()
{
	return endPointIndex;
}

void RouteSegmentResult::setSegmentTime( float segmentTime )
{
	this->segmentTime = segmentTime;
}

void RouteSegmentResult::setSegmentSpeed( float speed )
{
	this->speed = speed;
}

void RouteSegmentResult::setDistance( float distance )
{
	this->distance = distance;
}

void RouteSegmentResult::setEndPointIndex( int endPointIndex )
{
	this->endPointIndex=endPointIndex;
}

SHARED_PTR<RouteDataObject> RouteSegmentResult::getObject()
{
	return object;
}

#include "../utils/MathUtils.h"
float RouteSegmentResult::getBearing( int point,bool plus )
{
	return float(object->directionRoute(point,plus)/MathUtils::PI*180);
}

vector<RouteSegmentResult>& RouteSegmentResult::getAttachedRoutes( int routeInd )
{
	int st = MathUtils::abs(routeInd - startPointIndex);
	if (st<attachedRoutes.size())
	{
		return attachedRoutes[st];
	}
	static vector<RouteSegmentResult> list;
	return list;
}

float RouteSegmentResult::getBearingBegin()
{
	return (float)(object->directionRoute(startPointIndex,startPointIndex<endPointIndex)/MathUtils::PI*180);
}

void RouteSegmentResult::copyPreattachedRoutes( const RouteSegmentResult& toCopy, int shift )
{
	if(!toCopy.preAttachedRoutes.empty()) {
		int l = (int)toCopy.preAttachedRoutes.size() - shift;
		preAttachedRoutes.resize(l);
		for (size_t i=0;i<l;i++)
		{
			preAttachedRoutes[i]=toCopy.preAttachedRoutes[shift+i];
		}
	}
}

void RouteSegmentResult::attachRoute( int roadIndex,const RouteSegmentResult& r )
{
	int st = MathUtils::abs(roadIndex - startPointIndex);
	if (st>=attachedRoutes.size())
	{
		attachedRoutes.insert(attachedRoutes.end(),st+1-attachedRoutes.size(),std::vector<RouteSegmentResult>());
	}
...
float RouteSegmentResult::getBearingEnd()
{

	return (float) (MapUtils::alignAngleDifference(object->directionRoute(endPointIndex, startPointIndex > endPointIndex) - MathUtils::PI) / MathUtils::PI * 180);
}

float RouteSegmentResult::getDistance()
{
	return distance;
}

void RouteSegmentResult::setTurnType( const TurnType &turnType )
{
	this->turnType=turnType;
}

TurnType & RouteSegmentResult::getTurnType()
{
	return this->turnType;
}

void RouteSegmentResult::setDescription( const std::string &description )
{
	this->description = description;
}

std::string RouteSegmentResult::getDescription()
{
	return description;
}

float RouteSegmentResult::getSegmentSpeed()
{
	return speed;
}
