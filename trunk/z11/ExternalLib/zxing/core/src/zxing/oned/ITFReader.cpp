...
namespace zxing {
  namespace oned {
    ...
    void ITFReader::decodeMiddle(Ref<BitArray> row, int payloadStart, int payloadEnd,
      ...
      std::vector<int> counterDigitPair(counterDigitPairLen);//[counterDigitPairLen];
      ...
      while (payloadStart < payloadEnd) {
        ...
        if (!recordPattern(row, payloadStart, &counterDigitPair[0], counterDigitPairLen)) {
...
//      }
    ...
    int* ITFReader::findGuardPattern(Ref<BitArray> row, int rowOffset, const int pattern[],
      ...
      std::vector<int> counters(patternLength,0);//[patternLength];
    ...
    int ITFReader::decodeDigit(int counters[], int countersLen){
      ...
      for (int i = 0; i < max; i++) {
        std::vector<int> pattern(countersLen);//[countersLen];
