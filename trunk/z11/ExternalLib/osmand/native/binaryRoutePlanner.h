...
struct RouteSegmentResult {
	...
	SHARED_PTR<RouteDataObject> getObject();
	int getStartPointIndex();
	int getEndPointIndex();
	void setEndPointIndex(int endPointIndex);
	void setSegmentTime(float segmentTime);
	void setSegmentSpeed(float speed);
	float getSegmentSpeed();
	void setDistance(float distance);
	float getDistance();
	float getBearing(int point,bool plus);
	vector<RouteSegmentResult>& getAttachedRoutes(int routeInd);
	float getBearingBegin();
	float getBearingEnd();
	void copyPreattachedRoutes(const RouteSegmentResult& toCopy, int shift);
	void attachRoute(int roadIndex, const RouteSegmentResult& r);
...
struct RoutingConfiguration {
	...
	bool acceptLine(SHARED_PTR<RouteDataObject> r) {
		...
		for(; t != r->types.end(); t++) {
			...
			else if(type.first=="highway" ) {
				MAP_STR_FLOAT::iterator it=highwaySpeed.find(type.second);
				if(it!=highwaySpeed.end()&&it->second>0)
				{
					accepted = true;
				}
			}  
	...
	float defineSpeedPriority(SHARED_PTR<RouteDataObject> r) {
		...
		for(; t != r->types.end(); t++) {
			tag_value type = r->region->decodingRules[*t];
			if(highwayPriorities.find(type.first+"$"+type.second) != highwayPriorities.end()) {
...
// TODO FIX
	...
	float maxSpeed(SHARED_PTR<RouteDataObject> r) {
		...
		for(; t != r->types.end(); t++) 
		{
			if (r->region->decodingRules[*t].first=="maxspeed")
			{
				RouteTypeRule &rule=r->region->quickGetEncodingRule(*t);
				float f = rule.maxSpeed();
				if(f>0)
				{
					return f;
				}
	...
	float defineSpeed(SHARED_PTR<RouteDataObject> r) {
		...
		//std::vector<uint32_t>::iterator t = r->types.begin();
		//for(; t != r->types.end(); t++) {
		//	if(r->region->decodingRules[*t].first == "highway")
		//	{
		//		RouteTypeRule& type = r->region->quickGetEncodingRule(*t);
		//		if( type.getType() == RouteTypeRule::HIGHWAY_TYPE )
		//		{
		//			MAP_STR_FLOAT::iterator it=highwaySpeed.find(type.t+"$"+type.v);
		//			if(it != highwaySpeed.end())
		//			{
		//				return it->second/3.6;
		//			}
		//		}
		//	}
		//}
...
struct RoutingContext {
	...
	void loadHeaders(uint32_t xloc, uint32_t yloc) {
		...
		if (indexedSubregions.find(tileId) == indexedSubregions.end()) {
			...
			//ElapsedTimer searchRouteSubregionsTimer;
			//searchRouteSubregionsTimer.start();
			searchRouteSubregions(&q, tempResult);
			//searchRouteSubregionsTimer.pause();
			...
			//osmand_log_print(LOG_WARN, "[Native] searchRouteSubregionsTimer %d",searchRouteSubregionsTimer.getElapsedTime());
		}
		//ElapsedTimer loadHeaderObjectsTimer;
		//loadHeaderObjectsTimer.start();
		loadHeaderObjects(tileId);
		//loadHeaderObjectsTimer.pause();
		timeToLoad.pause();
		//osmand_log_print(LOG_WARN, "[Native] loadHeaderObjects %d",loadHeaderObjectsTimer.getElapsedTime());
	}
