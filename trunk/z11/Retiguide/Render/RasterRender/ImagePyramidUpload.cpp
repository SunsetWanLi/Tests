...
#include <iostream>
#include <sstream>
//#define MAX_TILES_COUNT_PER_FILE 1000
using namespace std;
using namespace z11;
...
int ImagePyramidUpload::getImagePyramidConfig(const string& map_name, const string& map_root_path, ImagePyramidConfig* image_pyramid_config)
{
	return getImagePyramidConfig(map_root_path + map_name, image_pyramid_config);
}

int ImagePyramidUpload::getImagePyramidConfig(const string& rooted_map_name, ImagePyramidConfig* image_pyramid_config)
{
	int ret = -1;
	try
	{
...
int ImagePyramidUpload::getPyramidConfig(const string& pyramid_config_name__, ImagePyramidConfig* pyramid_config)
{	
	string pyramid_config_name = pyramid_config_name__ + ".conf";
	if(NULL == pyramid_config)
		return -1;
	std::string config_name = pyramid_config_name__;
	...
	if (global_conf != NULL)
	{
		...
		if (strcmp(raster_type->value() ,"RT_GRID") == 0)
		{
			pyramid_config->raster_type = GIS_Embedded::RT_GRID;
		}
		else if (strcmp(raster_type->value() ,"RT_BMP") == 0)
		{
			pyramid_config->raster_type = GIS_Embedded::RT_BMP;
		}
		else if (strcmp(raster_type->value() ,"RT_JPG") == 0)
		{
			pyramid_config->raster_type = GIS_Embedded::RT_JPG;
		}
		else if (strcmp(raster_type->value() ,"RT_TIFF") == 0)
		{
			pyramid_config->raster_type = GIS_Embedded::RT_TIFF;
		}

		if (strcmp(repo_type->value() ,"REPO_FILES") == 0)
		{
			pyramid_config->repo_type = GIS_Embedded::REPO_FILES;
		}
		else if(strcmp(repo_type->value() ,"REPO_DATABASE") == 0)
		{
			pyramid_config->repo_type = GIS_Embedded::REPO_DATABASE;
		}
		...
		sscanf(regular_tile_size->value(), "%d", &(pyramid_config->m_regular_tile_size));
...
int ImagePyramidUpload::getImageCRSConfig(const string& image_crs_name__, ImagePyramidConfig* image_crs_config)
{
	//TODO:
...
void ImagePyramidUpload::getFloorInfo(const ImagePyramidConfig& image_pyramid_config, vector<ImagePyramidFloorInfo>* floors)
{
	...
	int regular_tile_size = image_pyramid_config.m_regular_tile_size;
	info.number = 0;
	...
	z11::uint_32 tiles_count_per_file = image_pyramid_config.m_tiles_count_per_file;
	info.tiles_count_per_file = tiles_count_per_file;//zhaoling
	...
	{
		info.number ++;
		info.width = static_cast<z11::uint_32>(ceil(1.0 * info.width / image_pyramid_config.m_level_factor));
		info.height = static_cast<z11::uint_32>(ceil(1.0 * info.height / image_pyramid_config.m_level_factor));
		info.regular_tile_number_per_row = info.width / regular_tile_size;
		info.non_regular_tile_number_per_row = (0 == info.width % regular_tile_size)?0:1;
		info.row_number = info.height / regular_tile_size;
...
string ImagePyramidUpload::getTileFileName(const vector<ImagePyramidFloorInfo>& floors, const string& map_name, int floor, int cur_upward_row, int cur_col)
{
	...
	z11::uint_32 tiles_count_per_file = floors[floor].tiles_count_per_file;//zhaoling

	return getTileFileName(map_name, floor, cur_upward_row, cur_col, total_rows, total_cols, tiles_count_per_file);
}

string ImagePyramidUpload::getTileFileName(const ImagePyramidFloorInfo& floor_info, const string& map_name, int cur_upward_row, int cur_col)
{
	...
	z11::uint_32 tiles_count_per_file = floor_info.tiles_count_per_file;//zholing
	return getTileFileName(map_name, floor_info.number, cur_upward_row, cur_col, total_rows, total_cols, tiles_count_per_file);
}


string ImagePyramidUpload::getTileFileName(const string& map_name, int floor, int cur_upward_row, int cur_col, int rows, int cols, z11::uint_32 tiles_count_per_file)
{
	...
	grand_parent_filename<<"/"<<map_name<<"/"<<map_name<<"_"<<floor;
	...
	string filename = grand_parent_filename.str() + parent_filename.str() + sub_filename.str();
	return filename;
}
}
