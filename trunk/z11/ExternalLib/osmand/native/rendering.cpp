...
#undef min
#undef max
extern void  drawTextOverCanvas(RenderingContext* rc, SkCanvas* cv);
extern void fillTextProperties(TextDrawInfo* info, RenderingRuleSearchRequest* render, float cx, float cy);
...
void drawIconsOverCanvas(RenderingContext* rc, SkCanvas* canvas)
{
	...
	std::vector<int> alreadyDrawnIcons(len);//int alreadyDrawnIcons[len];
	memset(&alreadyDrawnIcons[0], 0, sizeof(int)*len);//memset(alreadyDrawnIcons, 0, sizeof(int)*len);
	size_t ji = 0;
	SkPaint p;
