...
RenderingRule::RenderingRule(map<string, string>& attrs, RenderingRulesStorage* storage) {
	...
	for (; it != attrs.end(); it++) {
		...
		if (property->isString()) {
			...
			//add by GRF
			if (it->first=="color"&&!it->second.empty()&&it->second[0]=='$')
			{
				//find from renderingConstants
				map<std::string, std::string>::iterator constants_it=
					storage->renderingConstants.find(it->second.substr(1));
				if (constants_it!=storage->renderingConstants.end())
				{
					intProperties[i] = property->parseIntValue(constants_it->second);
				}
				else
...
RenderingRuleSearchRequest::RenderingRuleSearchRequest(RenderingRulesStorage* storage)  {
	...
	setBooleanFilter(PROPS->R_NO_POLOGONS, false);
	saveState();
}
